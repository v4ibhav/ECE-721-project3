...
void lsu::execute_load(cycle_t cycle,
  ...
  // If either a hit or a miss and the line has already been loaded
    ...
    try{
	  ...
	  uint64_t checkpoint_ID = proc->PAY.buf[LQ[lq_index].pay_index].checkPoint_ID;
...
void lsu::restore(unsigned int recover_lq_tail, bool recover_lq_tail_phase,
	...
	if(lq_tail_phase == recover_lq_tail_phase)
	{
		for(uint64_t i = recover_lq_tail;i<lq_tail;i++)
		{
		...
		for(uint64_t i = recover_lq_tail;i<lq_size;i++)
		{
			if(LQ[i].valid && LQ[i].addr_avail && !LQ[i].value_avail)
			{
				if(proc->PAY.buf[LQ[i].pay_index].C_valid)
